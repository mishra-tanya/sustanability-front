...
import imageURL from '/complete_bg.jpg';
...
export const CompleteGoals = async (data: CertificateData) => {
  ...
  try {
    ...
    doc.text(`Date of Certification: ${date}`, leftX, 190);

    const rightX = 190;
    doc.text(`Certificate Number: ${certificateNumber}`, rightX, 190);

    doc.save('completion-certificate.pdf');
  } catch (error) {
